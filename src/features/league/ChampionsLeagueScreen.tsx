...
/**
 * Champions-League-Ansicht (V7): Gruppenphase als Tabelle, danach der K.o.-Baum
 * (Achtel-, Viertel-, Halbfinale, Finale) als Runden-Grafik. Der eigene Klub
 * ist überall hervorgehoben. Die CL läuft bis zum Sieger, auch nach dem
 * Ausscheiden des Nutzers.
 */
...
function TeamLine({ state, id, goals, other, isWinner }: {
  state: ClState; id: string; goals: number | null; other: number | null; isWinner: boolean;
}) {
  const team = state.teams[id];
  const isUser = id === USER_CLUB_ID;
  const decided = goals !== null && other !== null;
  return (
...
function TieCard({ state, m }: { state: ClState; m: ClMatch }) {
  ...
  return (
    <Card style={[styles.tie, involvesUser && styles.tieUser]}>
      <TeamLine state={state} id={m.homeId} goals={m.played ? m.homeGoals : null} other={m.played ? m.awayGoals : null} isWinner={homeWin} />
      <View style={styles.tieDivider} />
      <TeamLine state={state} id={m.awayId} goals={m.played ? m.awayGoals : null} other={m.played ? m.homeGoals : null} isWinner={awayWin} />
    </Card>
  );
}
      ...
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <Card style={{ paddingVertical: spacing.sm }}>
          ...
          {table.map((row, i) => {
            ...
            return (
              <View key={row.clubId} style={[styles.tableRow, isUser && styles.userRow]}>
                <Text style={[styles.td, styles.colPos, advances && styles.advanceText]}>{i + 1}</Text>
...
const styles = StyleSheet.create({
  ...
  info: { color: colors.inkSoft, fontSize: font.body, marginBottom: spacing.md },
  section: {
